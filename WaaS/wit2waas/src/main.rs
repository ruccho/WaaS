...
fn process_dir(resolve: &Resolve, package_id: PackageId, out_dir: &str) -> Result<()> {
    ...
    for (name, id) in &package.worlds {
        ...
        source.open_block();
        {
            source.push_line(&format!("// world {name}"));
            source.push_docs(&world.docs);
            source.push_line(&format!("[global::WaaS.ComponentModel.Binding.ComponentInterface(@\"{}\")]", name));

            let mut line = format!("public partial interface I{}", to_upper_camel(name));

            let mut is_first = true;
            for (_key, item) in &world.exports {
                if let WorldItem::Interface {id, stability: _stability } = item {
                    if is_first {
                        is_first = false;
                        line.push_str(" : ");
                    } else {
                    ...
                    line.push_str(&interface.to_waas(resolve)?);
                }
            }
